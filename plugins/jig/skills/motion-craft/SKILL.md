...
- **3D 기울기 키프레임**(2026-09-20): keyframes `rotateX` / `rotateY`(deg) + `perspective`(시점 거리 px, 기본 1200·작을수록 강함). 카드가 세로축으로 뒤집혀 들어와 자리 잡기 = `rotateY 70→0` 450ms 급감속 + opacity 0→1 앞 1/3. 단순 뒤집기 등장은 프리셋 `flipY`/`flipX` 로 충분, **기운 채 머무는 패널**(살짝 누운 UI 화면)은 정지 값 `perspective{rotate_x,rotate_y,settle}` 또는 이 키프레임.

### 4-a. 제품 UI 모션 광고 문법 (SaaS 데모 필름, 2026-09-20)

> 이 장르를 통째로 만들 땐 전용 스킬 `ui-demo-film`(장면표·빌더 코드·에셋 제작·완료 조건)을 읽는다. 아래는 문법 요약이다.

"흰 박스에 글자 한 줄"을 장면마다 지웠다 띄우면 파워포인트가 된다(실제 반려: "그림판으로 편집한 느낌"). 레퍼런스 급이 되는 조건은 에셋이 아니라 아래 여섯 가지다.

1. **카메라 한 대** — 장면을 끊지 말고 잇는다. 보이지 않는 카메라 null(도형 opacity 0)에 `scale`·`x`·`y` 키프레임을 주고 그 장면의 UI 조각 전부를 `attach_to` 로 붙인다. 넓은 화면 → 버튼 하나로 0.3초 줌(scale 1→5, 급감속 `0.16,1,0.3,1`), 나머지는 같은 움직임으로 화면 밖으로 밀려나며 fade. **앞 장면의 작은 요소가 다음 장면의 주인공**이 되게 설계한다.
2. **UI 밀도** — 패널 하나에 읽히지 않아도 되는 디테일 15~30개: 사이드바 아이콘 점, 검색창, 필터 칩, 색 다른 아바타 점 3~4개, 회색 글줄 막대(높이 6~8px, 폭 제각각), 점 3개 메뉴. 전부 shape/frame 레이어. 가능하면 **실제 제품 화면을 그대로 옮긴다**.
3. **화면 밖에 걸치기** — 주인공 조각은 크게, 한쪽이 잘리게 놓는다(알약의 오른쪽 40% 가 화면 밖). 전부 가운데 정렬·전부 화면 안 = 슬라이드.
4. **완전 정지 금지** — 머무는 구간에도 카메라 null 이 3~4초에 20~40px 느리게 민다(linear). 로딩은 점이 아니라 도는 호(path `trimStart/trimEnd` + rotation 루프) + 가운데 반짝이 pulse.
5. **헤드라인은 채워진다** — 큰 slideUp 금지. 줄마다 따로 레이어, 5~8px 만 올라오며 회색(#9aa3af)→먹색, 둘째 줄은 100ms 늦게. 이동 거리보다 **색이 차오르는 것**이 읽힌다.
6. **커서는 곡선·화면 밖 출입** — 화면 밖에서 비스듬히 들어와 감속 도착(모션 경로 3점, smooth), 클릭 = 버튼 scale 1→0.94→1.03→1 + 커서 0.85 딥, 그 뒤 다시 화면 밖으로. 입력창은 글이 두 줄이 되는 순간 height 키프레임으로 자라고 캐럿(폭 3px rect, flicker 530ms)이 따라간다.

그림자는 검정이 아니라 **브랜드 색 넓은 글로우**(dropShadow radius 60, y 14, 강조색 α0.22). 결과물(카드·노드)은 한 번에 뜨지 않고 **빈 테두리 → 채움 → 체크 점** 순으로 자란다, 마지막 하나만 다른 색.
